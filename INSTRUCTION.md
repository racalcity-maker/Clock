...
- Перейдите в `RAD `.
- Короткое `NEXT/PREV` — переключение сохранённых станций.
- Длинное `NEXT/PREV` — автоскан и сохранение станций (только если список пуст).
- Частота показывается ~10 секунд после входа/смены, затем отображаются часы.

Если плохо ловит — проверьте антенну и удалите модуль от источников помех (LED‑лента, питание).

## 11) Wi-Fi (ручная настройка)
...
- Радио не ловит: проверьте антенну и помехи рядом с модулем.
