...
Проект: ESP32‑устройство «часы + аудио» с 4‑разрядным 7‑сегментным дисплеем, SD‑плеером, Bluetooth A2DP приёмником, FM‑радио и будильником.
...
- В `RAD ` будильник временно глушит радио и играет MP3/тон, затем возвращает радио.
...
- Короткое нажатие `NEXT/PREV` в режиме `RAD ` переключает сохранённые станции.
- Длинное нажатие `NEXT/PREV` запускает автоскан и сохраняет найденные станции в NVS (только если список пуст).
- Частота показывается ~10 секунд после входа в режим или смены станции, дальше отображаются часы.
...
- Root: `CLCK` / `PLYR` / `BLUE` / `RAD ` / `EqUA` / `SEt `.
...
- **FM не ловит**: проверьте антенну и расстояние до помех (LED‑лента/питание/провода I2C).
